...
void init_board(char board[][GRID_NUM])
{
    ...
    for(int i = 0; i< 21 ; i++)
    {
        board[i][0] = board[0][i] = board[i][GRID_NUM-1] = board[GRID_NUM-1][i] = BORDER;//最外圈（第 0 行、第 20 行、第 0 列、第 20 列）赋值为 BORDER（边界标记），永远不能落子；
    }
    for(int i = 1 ; i< GRID_NUM-1 ; i++)
    {
        for(int j = 1 ; j < GRID_NUM-1 ; j++)
        {
            board[i][j] = NOSTONE;//初始化为 NOSTONE（空棋）。
        }
    }
}
//落子与撤销落子
...
int log_to_file(char* msg) 
{
    ...
    if (file == NULL)
    {
        printf("Error: Can't open log fizle - %s\n", g_log_file_name);
        return -1;
    }
...
{// // 单子情况：两个坐标相同（黑棋第一手）
    ...
    {
        msg[1] = 'S' - move->positions[0].x + 1;//修改
        msg[0] = move->positions[0].y + 'A' - 1;//修改
        msg[2] = 0;
        return 1;
    } 
    else
